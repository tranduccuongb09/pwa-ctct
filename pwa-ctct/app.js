...
function renderQuestion(){
  ...
  box.setAttribute('role','radiogroup'); // ★ hỗ trợ screen reader
  box.innerHTML = '';

  const opts = q.options || {}; // ★ an toàn nếu thiếu options
  ['A','B','C','D'].forEach(k=>{
  ...
  // ★ cuộn về đầu khung câu hỏi cho dễ nhìn (đặc biệt mobile)
  const quizCard = document.getElementById('quizBox');
...
function startTimer(){
  ...
  const tick = () => {
    ...
    if (remainingSeconds <= 0){ // ★ dừng ngay tại 00:00
      clearInterval(timer);
      submitQuiz();
      return;
    }
